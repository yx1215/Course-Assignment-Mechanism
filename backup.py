...
class ProbabilityGenerator:
    ...
    def reduce_one_path(self):
        ...
        i, j = self.find_first_non_zero(first, 0, visited)
        ...
        while True:
            ...
            try:
                ...
                if visited_row[i] and visited_column[j]:
                    if count % 2:
                        for u, v in even_list:
                            if i == u:
                                idx = chosen_list.index((u, v))
                                break
                    else:
                        for u, v in odd_list:
                            if j == v:
        ...
        odd_list = chosen_list[::2]
        even_list = chosen_list[1::2]
        min_to_give_by_circle = self.find_min_to_decrease(odd_list, even_list, student_per_class)
        min_to_get_by_circle = self.find_min_to_increase(odd_list, even_list, student_per_class)
...
if __name__ == '__main__':
    machine = ProbabilityGenerator(100, 200)
